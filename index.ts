...
async function searchBrave(
  ...
  maxResults: number,
  apiKey: string
): Promise<{ results: BraveSearchResult[]; used: "brave" }> {
  const url = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=${Math.min(maxResults, 50)}&search_lang=en`;
...
async function handleSocialSearch(
  ...
  try {
    const brave = await searchBrave(searchQuery, maxResults, env.BRAVE_API_KEY);
    searchResults = brave.results.map((r) => ({
      title: r.title,
      url: r.url,
      snippet: r.description ?? "",
    }));
    sourceApi = brave.used;
  } catch (braveError) {
    attempts = 2;
    try {
      const serper = await searchSerper(searchQuery, maxResults, env.SERPER_API_KEY);
      searchResults = serper.results.map((r) => ({
        title: r.title,
        url: r.link,
        snippet: r.snippet ?? "",
      }));
      ...
      throw Object.assign(
        new Error("Both Brave and Serper search APIs failed. Please try again later."),
        { code: "SEARCH_API_UNAVAILABLE", details: {
          brave_error: braveError instanceof Error ? braveError.message : String(braveError),
...
Search results:
${JSON.stringify(searchResults.slice(0, maxResults))}
...
Extract as many relevant profiles as possible. Set confidence based on relevance (1.0 = exact match, 0.3 = tangential).`;
  ...
  try {
    ...
    const cleaned = llmOutput.replace(/```json\s*/gi, "").replace(/```\s*/g, "").trim();
    const parsed = JSON.parse(cleaned);
    return { data: parsed, modelUsed: `${MODELS.chat}+${sourceApi}`, attempts };
  } catch {
    return {
      data: {
        profiles: searchResults.slice(0, maxResults).map((r) => ({
          name: r.title,
          url: r.url,
          platform: "unknown",
          snippet: r.snippet,
      ...
      modelUsed: `${sourceApi} (raw, LLM normalization failed)`,
      attempts,
    };
  }
}
